...
**Amended 2026-09-03:** Steven lifted this rule for his own build. The app is a personal tool that is not distributed, so bundling Apple artwork is his call. In practice nothing was copied anyway: the MacBook Pro runs iTunes 12.9.5, whose icon is the modern white one, and no iTunes 10 icon file exists on either machine. The app icon is therefore still drawn in code by `client/Tools/MakeIcon`, matching the reference he supplied.

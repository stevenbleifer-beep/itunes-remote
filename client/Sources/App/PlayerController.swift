...
final class PlayerController {
    ...
    /// How far before the end of a song the next one is asked for, so that
    /// it lands as this one runs out and the gap between songs is as small
    /// as a fresh `play` allows. There is nothing to beat: a track played by
    /// reference is a one-off to iTunes, and with nothing standing behind it
    /// iTunes simply stops when it ends (see `sourceStale` for the case where
    /// something *is* standing behind it). So the lead is just the round trip
    /// of the app's own last `play` — a tenth of a second of HTTP and three
    /// of iTunes finding the track by persistent ID in a 93,000-track
    /// library, or more over the Tailscale tunnel — plus the age of the poll
    /// the timer is aimed from (aimed by the round trip alone, iTunes sat
    /// stopped for half a second between songs), capped so that a slow
    /// moment on the old machine never clips seconds off the end of a song.
    /// Late by a little means a moment's silence; early by a little means
    /// the last fraction of a second, which is silence on nearly everything.
    private var endLead: Double { min(1.5, max(0.4, lastPlayLatency + 0.3)) }
        ...
        do {
            ...
            if let expected = expectedTrack, let now = s.track?.persistentId, now != expected, s.isPlaying,
               lastRemote?.id == expected {
                ...
                // behind it — a playlist or album it was once told to play,
                // which it keeps across stops, relaunches and days, and
                // resumes after every one-off. That is where every stray
                // song came from. The next play replaces it (see
                // `sourceStale`), and this song is treated as finished.
                Self.trace("iTunes went on to \(s.track?.name ?? now) by itself — a stale source stands behind our songs; the next play will replace it")
                sourceStale = true
                expectedTrack = nil
                remoteState = s
                lastPoll = Date()
    ...
    func adopt(_ id: String) {
        ...
        expectedTrack = id
        finishedTrack = nil
        // Whatever started this song may have left a source standing behind
        // it (a song put on at the MacBook Pro plays from a playlist or an
        // album). The first handoff replaces it rather than finding out.
        sourceStale = true
    }

    /// True when iTunes may be holding a *source* — a playlist or album it
    /// was told to play as a whole — behind the songs this app plays.
    ///
    /// What iTunes does at the end of a song was measured, not guessed
    /// (HANDOFF.md, "What iTunes actually does"): a track played by
    /// reference, from a playlist or from the library, is a one-off, and
    /// with nothing standing behind it iTunes stops when it ends. But
    /// `play <playlist>` — or a bare `play` with nothing current, which plays
    /// whatever is selected in iTunes' own window — loads a source that
    /// persists across stops, relaunches and days, and every one-off played
    /// afterwards is followed by the next song out of it. That was the
    /// "random song in between".
    ///
    /// The only way to clear a source is to play another one out. So when a
    /// stale source shows itself (iTunes moves on to a song nobody here
    /// asked for), the next play goes through the daemon's queue playlist:
    /// one song, played as a playlist, which makes that one song the whole
    /// source. Interrupted or played out, it is consumed either way, and
    ...
    private func sendPendingPlay() {
        ...
        let asked = Date()
        let flush = sourceStale
        command { [weak self] in
            if flush {
                do {
                    // The song as a one-song playlist: it replaces whatever
                    // source iTunes was holding, and is consumed with it.
                    let reply = try await api.queuePlay(tracks: [p.track])
                    self?.sourceStale = false
                    Self.trace("played as a one-song playlist (\(reply.playlist)) to replace the stale source")
                } catch {
                    // An old daemon, or iTunes said no: a one-off still plays
                    // the right song, and the source stays marked for the
                    // next try.
                    Self.trace("could not replace the stale source (\(error.localizedDescription)) — playing the song by itself")
